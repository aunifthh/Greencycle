                        ...
                        <table id="pickupTable" class="table table-bordered table-hover">
                            ...
                            <%
                                if (requests != null && !requests.isEmpty()) {
                                    for (int i = 0; i < requests.size(); i++) {
                                        ...
                                        if (r.getPlasticWeight() > 0) stmt: { itemList.append("Plastic (").append(df.format(r.getPlasticWeight())).append("kg)"); }
                                        ...
                                        else if ("Completed".equals(status) || "Payment Completed".equals(status)) { badgeClass = "badge-success"; }
                            ...
                            %>
                                <tr data-index="<%= i %>">
                                    ...
                                    <td><span class="badge <%= badgeClass %> badge-status status"><%= status %></span></td>
                                   ...
                                   <td>
                                    <% 
                                        ...
                                        if (pickupDate != null) { canCancel = pickupDate.isAfter(today.plusDays(1)); }
                                    %>

                                    <% if ("Pending Pickup".equals(status)) { %>
                                        <div style="margin-bottom:5px;">
                                        <form method="post" action="${pageContext.request.contextPath}/RequestServlet?action=cancel_pickup" style="display:inline-block; width:100%; text-align:center;">
                                            <input type="hidden" name="requestID" value="<%= r.getRequestID() %>">
                                            <button type="button" class="btn btn-sm btn-block <%= canCancel ? "btn-danger cancelBtn" : "btn-secondary" %>" <%= canCancel ? "" : "disabled" %> data-cancancel="<%= canCancel %>" data-date="<%= pickupDate %>">
                                    ...
                                    <% if ("Quoted".equals(status) && totalAmount > 0) { %>
                                        <div>
                                            <a class="btn btn-info btn-sm btn-block" href="${pageContext.request.contextPath}/customer/quotation.jsp?requestID=<%= r.getRequestID() %>">
