...
<% 
    ...
    List<RequestBean> requests = (List<RequestBean>) request.getAttribute("requestList"); 
%>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                        <table id="mgmtTable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Customer</th>
                            ...
                            <tbody>
                                <% if (requests !=null) { for (RequestBean req : requests) { boolean hasDate=req.getPickupDate() !=null; %>
                                <tr>
                                    <td><%= req.getRequestID() %></td>
                                    <td><%= req.getCustomerID() %></td>
                                    <td><%= req.getFullAddress() %></td>
                                    <td>
                                        <span class="badge badge-<%= req.getStatus().equals("Completed") ? 
                                            "success" : 
                                    ...
                                    <td><%= hasDate ? req.getPickupDate() + " " + req.getPickupTime() : "Not Set" %></td>
                                    <td><%= req.getPlasticWeight() %> / <%= req.getPaperWeight() %> / <%= req.getMetalWeight() %></td>
                                    <td>
                                        <%-- Staff Actions --%>
                                        <% if ("staff".equals(role)) { %>
                                            <% if ("Pending Pickup".equals(req.getStatus())) { %>
                                                <button class="btn btn-warning btn-sm" onclick="openReweigh(<%= req.getRequestID() %>)">
                                                    <i class="fas fa-balance-scale"></i> Reweigh
                                                </button>
                                            <% } %>
                                        <% } %>
            ...
            <form action="${pageContext.request.contextPath}/RequestServlet" method="post">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" name="plasticWeight" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Paper Weight (kg)</label>
                        <input type="number" step="0.01" name="paperWeight" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Metal Weight (kg)</label>
                        <input type="number" step="0.01" name="metalWeight" class="form-control" required>
                    </div>
                </div>
